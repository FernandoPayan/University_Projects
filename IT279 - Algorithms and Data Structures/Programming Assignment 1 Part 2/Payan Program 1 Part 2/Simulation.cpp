...
void checkCustomers(Queue theQueue, Register theRegister, double averageWait, int maxWait, int counter, ofstream singleFile);
...
int main(int argc, char **argv)
{
    ...
    ofstream singleFile; singleFile.open("singleline.txt");
    ofstream multipleFile; multipleFile.open("multiplelines.txt");
...
void checkCustomers(Queue theQueue, Register theRegister, double averageWait, int maxWait, int counter, ofstream singleFile)
{
    averageWait += (counter - theQueue.front().arrivalTime); // Adds to the wait time

    if (maxWait < (counter - theQueue.front().arrivalTime)) // Gets the max wait time
    {
        maxWait = (counter - theQueue.front().arrivalTime);
    }

    theRegister.doService(theQueue.dequeue(), 1, counter, singleFile) // Dequeues customer from queue and sends to register 
}