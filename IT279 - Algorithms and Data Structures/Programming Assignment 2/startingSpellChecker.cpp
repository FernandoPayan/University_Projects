...
void checkSpelling(const string& inFileName, const Dictionary& dict)
{
    ...
    while (!infile.eof())
    {
        ...
        while (iss >> curWord) // Iterates through each word in the line
        {
            ...
            if (word.length() >= 1) // Ensures no empty string
                if (!dict.findEntry(makeLowerCase(word)))
                {
                    cout << word << " on line " << lineNum << endl;
                    findSuggestions(word, dict);
                }
        }
...
string addWord(string curWord, Dictionary& dict)
{
    ...
    {
        cout << "CURWORD'S LENGTH IS : " << i << endl;
        ...
        if (i == (curWord.length() - 1))
        {
            if (isalpha(curWord[i])) // Checks if the last word is a letter
            {
                ...
                {
                    cout << "FROM SPELLCHECKER: GOING TO DO ADD ENTRY ON " << word << endl;
                    dict.addEntry(new string((word))); // Adds the word to the dictionary
                }
                //resets the word
                word = "";
                cout << "END OF ADD ENTRY. MOVING ONTO NEXT WORD..." << endl;
            }
        } else if (!isalpha(curWord[i])) // If the character is not a letter
        {
            cout << "RUNNING FIND ENTRY" << endl;
            ...
            cout << "WORD IS CURRENTLY: " << word << endl;
    }
