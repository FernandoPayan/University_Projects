...
{
    Node* curNode = root; // Used to keep track of the current node
    while (curNode && *(curNode->data) != key)
    {
        if (*(curNode->data) > key) // If the data is > key value, we go to the left child
        {
            curNode = curNode->left;
        }
        else // If the data is < key value, we go to the right child
        {
...
{
    if (root)
        printInOrder(outputStream, root); // Prints the dictionary in order
}

void Dictionary::printDictionaryKeys(ostream& outputStream) const
{
    if (root)
        printTree(outputStream, root, 0); // Prints the dictionary keys
}
...
void Dictionary::clear(Node* node)
{
    if (node)
    {
        clear(node->left); // Recursively clears all the values to the left
        clear(node->right); // Recursively clears all the values to the right
        delete node->data; 
        delete node; 
    }
}
...
Dictionary::Node* Dictionary::copy(Dictionary::Node* origNode)
{
    if (origNode)
    {
        string* newData = new string(*(origNode->data)); // Stores the string from the node's data.
...
void Dictionary::printInOrder(ostream& outputStream, Node* curNode) const
{
    ...
    if (curNode->left) // If we have a left child, we will recursively print the left children
        printInOrder(outputStream, curNode->left);
    outputStream << (*(curNode->data)) << endOfLine;
    if (curNode->right) // After checking the left side, we will check if we have a right child and recursively print the right children
