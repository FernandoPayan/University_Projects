    ...
    else if (argc == 2) /* BATCH MODE */
    {
        ...
        while (getline(&input, &length, input_file) != -1)
        {
            ...
            else if (strstr(parsed_input[0], "path"))
            {
                ...
                else /* Overwrite the path directory and the path list */
                {
                    /* Put the arguments into our input copy list*/
                    strcpy(input_copy, parsed_input[1]);

                    /* Make a temp copy of the input_copy */
                    char *temp[200];
                    strcpy(temp, input_copy);

                    /* Copy the path to our path directory*/
                    strcpy(path_dir, input_copy);
                    ...
                    /* Iterate through all the arguments and add them to the path list */
                    while (token != NULL)
                    {
                        path[index] = token;
                        num_paths++;
