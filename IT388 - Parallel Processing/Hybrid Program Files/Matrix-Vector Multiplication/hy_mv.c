/* File:  hy_mv.c
 *
 * Purpose:  Hybrid Parallel matrix-vector multiplication
 *
 * Compile:  mpicc -g -Wall -o mv hy_mv.c
...
# pragma omp parallel private(j,local_i)
   {
#pragma omp single
...
# pragma omp  for
    ...
    // Update the priting statement, should print number processors, number threads
    if (my_rank == 0) {
            printf("\n N= %d, #processors: %d, #threads: %d, elapsed %lf sec\n",m,nproc,numThreads,elapsed);
    }
