...
Author: Fernando Payan
 Processing array using collective communication.
 
 Complete items 1 through 9.

Compile:
    mpicc -g -Wall -o mpi_array mpi_array.c
Execution:
    mpiexec -n 4 ./mpi_array
...
int main(int argc, char* argv[]){
    ...
    if (my_rank == 0)
    {
       ...
       for (dest = 1; dest < nproc; dest++) // This is where the fatal error is occuring; where 4 < 4 is passing.
       {
            offset = work * dest; // Doing this because the offset is respective to the destination
            printf("DESTINATION VALUE: %d\n", dest);
            MPI_Send(&array[offset], work, MPI_INT, dest, 13, MPI_COMM_WORLD);
       }
